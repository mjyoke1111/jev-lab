...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 const questions=Object.fromEntries(rows.map((row,i)=>[`row_${i}`,{type:'choice',instructions:`Apply this policy to the row. Policy: ${policy}`,criteria:{keep:'The row clearly satisfies the policy and should be kept.',review:'The row is ambiguous, incomplete, or needs human review under the policy.',drop:'The row clearly violates the policy and should be dropped.'}}]));
 ...
 const decisions=rows.map((row,i)=>{const a=body.answers?.[`row_${i}`]||{};const probabilities=a.probabilities||a.distribution||{};return{id:row.id,rowHash:row.hash,decision:String(a.choice||'review'),probabilities,distributionConcentration:Number(a.confidence||0)}});
