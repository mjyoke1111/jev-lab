...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 const r=await fetch('https://api.typesafe.ai/v1/systemone',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'jev-1.13.0',state:{query:f.query,candidates:f.candidates},questions:{best:{type:'choice',instructions:'Which candidate best answers the query? Choose only from the supplied candidate ids.',criteria:Object.fromEntries(f.candidates.map(c=>[c.id,`${c.title}: ${c.excerpt}`]))}}})});
