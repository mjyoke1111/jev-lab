...
def call(state, questions, tag):
    ...
    open('batch1.jsonl','a').write(json.dumps(rec)+"\n")
    return rec
STATES=[
...
Q={
 ...
 "frustration":{"type":"score","instructions":"How frustrated is the customer?","criteria":{"1":"Calm or positive","2":"Mildly concerned","3":"Clearly frustrated","4":"Angry","5":"Furious, threatening to leave"}},
 "churn_risk":{"type":"noul","instructions":"Is the customer at risk of cancelling?"},
}
for i,s in enumerate(STATES):
    call(s,Q,f"batched:s{i}")
    for k,v in Q.items(): call(s,{k:v},f"single:s{i}:{k}")
for i in (0,5):
